...
function showPreview(rt, formid) {
	...
	while (form && form.tagName && form.tagName.toLowerCase() != 'form') form = form.parentNode;
	var url = document.location.href;
	if (url.indexOf('?') > -1) url += '&subtask=preview&formid='+formid;
	else if (url.toLowerCase().substr(url.length-4, 4) == '.php' || url.toLowerCase().substr(url.length-4, 4) == '.htm' || url.toLowerCase().substr(url.length-5, 5) == '.html') url += '?subtask=preview&formid='+formid;
	else url += '/?subtask=preview&formid='+formid;
	...
	if (!previewTitle) var previewTitle = '<h1>Form Preview</h1>';
	...
	var cssURL = document.location.href;
	if (cssURL.indexOf('?') > -1) cssURL += '&subtask=js&object=css&formid='+formid;
	else if (cssURL.toLowerCase().substr(cssURL.length-4, 4) == '.php' || cssURL.toLowerCase().substr(cssURL.length-4, 4) == '.htm' || cssURL.toLowerCase().substr(cssURL.length-5, 5) == '.html') cssURL += '?subtask=js&object=css&formid='+formid;
	else cssURL += '/?subtask=js&object=css&formid='+formid;
	var tmp = '';
