...
<div id="layout" data-field-name="<?php echo $name ?>">
    ...
    <div data-layout="<?php echo $layout ?>">
        ...
        <div data-assignment class="uk-scrollable-box uk-margin tm-width tm-assign-select">
            ...
            <?php foreach ($menu->links as $i => $link) : ?>
                <li>
                    <label>
                        <input value="<?php echo $link->value ?>" name="<?php echo "{$name}[{$layout}][assignment][]" ?>" type="checkbox"<?php if (@in_array($link->value, $layouts[$layout]['assignment'])) echo ' checked="checked"' ?>> <?php echo $link->text ?>
                    </label>
                    <?php
