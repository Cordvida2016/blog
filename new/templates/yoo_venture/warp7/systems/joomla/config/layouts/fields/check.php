...
// check DebugPluginDisabled
$this['check']->checkDebugPluginDisabled();
...
if ($critical || $notice) {
	...
	echo implode('', array_map(function($message) {
		return "<li class=\"uk-text-danger\"><i class=\"uk-icon uk-icon-bolt\"></i> {$message}</li>";
	}, $critical));

	echo implode('', array_map(function($message) {
		return "<li class=\"uk-text-warning\"><i class=\"uk-icon uk-icon-bolt\"></i> {$message}</li>";
	}, $notice));
