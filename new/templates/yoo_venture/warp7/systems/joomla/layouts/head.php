...
if ($this['config']->get('bootstrap', true)) {
	if ($this['config']['direction'] == 'rtl' && $bootstrap = $this['path']->url('css:bootstrap-rtl.css')) {
        $styles->prepend($this['asset']->createFile($bootstrap));
	} else if ($bootstrap = $this['path']->url('css:bootstrap.css')) {
		$styles->prepend($this['asset']->createFile($bootstrap));
	}
}
...
if ($this['config']['customizer']) {
	foreach ($styles as $style) {
		...
		if ($this['path']->path($file)) {
			$styles->add($this['asset']->createString('', array_merge($style->getOptions(), array('data-file' => basename($style->getPath())))));
		}
	}
}
// developer mode
	...
	foreach ($styles as $style) {
		...
		if ($this['path']->path($file)) {
            ...
            $files[] = array('target' => basename($style->getPath()), 'source' => $this['asset']->createString($this['asset']->createFile($file)->getContent($filter).PHP_EOL.$this['asset']->createFile(sprintf('less:styles/%s.less', $this['config']['style']))->getContent(), array_merge($style->getOptions(), array('type' => 'text/less')))->getContent());
            $styles->remove($style);
        }
    }
...
// compress styles and scripts
else if ($compression = $this['config']['compression']) {
	...
	if ($styles) {
		$styles = array($this['asset']->cache('theme.css', $styles, $filters, $options));
...
if ($styles) {
	foreach ($styles as $style) {
		if ($url = $style->getUrl()) {
			printf("<link %srel=\"stylesheet\" href=\"%s\">\n", $this['config']['customizer'] ? 'data-theme ' : '', $url);
		} else {
