...
function saveForm() {
	...
	var tmp = document.sfdFormName;
	tmp = tmp.replace(/&/g,'#amp#');
	tmp = tmp.replace(/\+/g,'#plus#');
	tmp = tmp.replace(/\'/g,'#quot#');
	tmp = tmp.replace(/\%/g,'#pr#');
	tmp = tmp.replace(/\$/g,'#ss#');

	s2+='&name='+tmp;
	var tmp = document.sfdFormDescription;
	tmp = tmp.replace(/&/g,'#amp#');
	tmp = tmp.replace(/\+/g,'#plus#');
	tmp = tmp.replace(/\'/g,'#quot#');
	tmp = tmp.replace(/\%/g,'#pr#');
	...
	var url = document.siteURL+'/administrator/index.php?option=com_sfg&task=saveform&fid='+document.form_id;
...
function loadForm() {
	showLoadingImage();
	var xmlDoc = loadXMLFile(document.siteURL+'/administrator/index.php?option=com_sfg&task=loadform&fid='+document.form_id+'&tmp='+Math.random());
	...
	if (tmp) document.formExternalAccess = Array(tmp.substr(0,1)-0, tmp.substr(1,1)-0);
