...
if (!class_exists('JHttpTransportStream') AND method_exists('JHttpTransport', 'isSupported'))
{
	...
	public function request($method, JUri $uri, $data = null, array $headers = null, $timeout = null, $userAgent = null)
	{
		...
		$context = stream_context_create(
			array(
				'http' => $options,
				'ssl' => array(
					'verify_peer'   => $this->options->get('verify_peer', PWEB_JHTTP_VERIFY_PEER),
					'cafile'        => $this->options->get('certpath', dirname(__FILE__) . '/cacert.pem'),
					'verify_depth'  => 5,
				)
			)
		);
