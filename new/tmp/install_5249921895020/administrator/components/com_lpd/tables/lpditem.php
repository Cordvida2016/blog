<?php
/**
 * @version		$Id: lpditem.php 1338 2011-11-25 15:59:49Z lefteris.kavadas $
...
class TableLPDItem extends JTable {
	...
	var $customcss = null;
	var $customjs = null;
	var $googleanalyticsscript = null;
	var $gwocontrolscript = null;
	var $gwotrackingscript = null;
	var $gwoconversionscript = null;
	var $params = null;
	var $plugins = null;
	var $language = null;
	...
	function __construct(&$db) {
		...
		parent::__construct('#__lpd_items', 'id', $db);
	}
	...
	function check() {
		...
		if (trim($this->name) == '') {
			$this->setError(JText::_('LPD_ITEM_MUST_HAVE_A_NAME'));
			return false;
		}
		if (trim($this->title) == '') {
			$this->setError(JText::_('LPD_ITEM_MUST_HAVE_A_TITLE'));
			return false;
		}
		if (!$this->catid) {
			$this->setError(JText::_('LPD_ITEM_MUST_HAVE_A_CATEGORY'));
			return false;
		}
		if ( empty($this->alias)) {
	...
	function getNextOrder($where = '', $column = 'ordering') {
		...
		$query = "SELECT MAX({$column}) FROM #__lpd_items";
	...
	function reorder($where = '', $column = 'ordering') {
		...
		$k = $this->_tbl_key;
		$query = "SELECT {$this->_tbl_key}, {$column} FROM #__lpd_items WHERE {$column}>0";
		...
		for ($i = 0, $n = count($orders); $i < $n; $i++) {
			if ($orders[$i]->$column >= 0) {
				if ($orders[$i]->$column != $i + 1) {
					$orders[$i]->$column = $i + 1;
					$query = "UPDATE #__lpd_items SET {$column}=".(int) $orders[$i]->$column;
