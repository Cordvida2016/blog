...
class LPDModelUsers extends JModel
{
	...
	function getData() {
		...
		$filter_group_lpd = $mainframe->getUserStateFromRequest($option.$view.'filter_group_lpd', 'filter_group_lpd', '', 'string');
		$search = $mainframe->getUserStateFromRequest($option.$view.'search', 'search', '', 'string');
		$search = JString::strtolower($search);

		$query = "SELECT juser.*, lpduser.group, lpdgroup.name as groupname FROM #__users as juser ".
		"LEFT JOIN #__lpd_users as lpduser ON juser.id=lpduser.userID ".
		"LEFT JOIN #__lpd_user_groups as lpdgroup ON lpduser.group=lpdgroup.id ";
		...
		if ($filter_group_lpd) {
			$query .= " AND lpduser.group = ".$db->Quote($filter_group_lpd);
		}
	...
	function remove() {
		...
		$db = &JFactory::getDBO();
		$query = "DELETE FROM #__lpd_users WHERE userID IN(".implode(',', $cid).")";
		$db->setQuery($query);
		$db->query();
		$cache = & JFactory::getCache('com_lpd');
		$cache->clean();
		$mainframe->redirect('index.php?option=com_lpd&view=users', JText::_('LPD_USER_PROFILE_DELETED'));
	}
	...
	function hasProfile($id){
		...
		$db = & JFactory::getDBO();
		$query = "SELECT id FROM #__lpd_users WHERE userID = ".(int)$id;
		$db->setQuery( $query );
		$result = $db->loadResult();
		return $result;
	}
	...
	function enable(){
		...
		$db->setQuery( $query );
		$db->query();
		$mainframe->redirect('index.php?option=com_lpd&view=users', JText::_('LPD_USERS_ENABLED'));
	}
	...
	function disable(){
		...
		$db->setQuery( $query );
		$db->query();
		$mainframe->redirect('index.php?option=com_lpd&view=users', JText::_('LPD_USERS_DISABLED'));
	}
	...
	function delete(){
		...
		if(in_array($user->id, $cid)) {
			$mainframe->enqueueMessage(JText::_('LPD_YOU_CANNOT_DELETE_YOURSELF'), 'notice');
		}
		if(LPD_JVERSION == '16') {
		...
		$query = "DELETE FROM #__lpd_users WHERE userID IN(".implode(',',$IDsToDelete).") AND userID!={$user->id}";
		$db->setQuery( $query );
		$db->query();

		$mainframe->redirect('index.php?option=com_lpd&view=users', JText::_('LPD_DELETE_COMPLETED'));
	}
	...
	function saveMove(){
		...
		$lpdgroup = JRequest::getInt('lpdgroup');
		if(LPD_JVERSION == '16') {
		...
		if($lpdgroup){
			foreach($cid as $id) {
				$query = "SELECT COUNT(*) FROM #__lpd_users WHERE userID = ".$id;
				$db->setQuery($query);
				$result = $db->loadResult();
				if($result) {
					$query = "UPDATE #__lpd_users SET `group`={$lpdgroup} WHERE userID = ".$id;
				}
				else {
					$user = JFactory::getUser($id);
					$query = "INSERT INTO #__lpd_users VALUES ('', {$id}, {$db->Quote($user->username)}, '', '', '', '', {$lpdgroup}, '')";
				}
				$db->setQuery( $query );
				$db->query();
			}
		}
		$mainframe->redirect('index.php?option=com_lpd&view=users', JText::_('LPD_MOVE_COMPLETED'));
	...
	function import() {
		...
		foreach($usergroups as $usergroup){
			$LPDUserGroup = &JTable::getInstance('LPDUserGroup', 'Table');
			$LPDUserGroup->name = JString::trim($usergroup->name)." (Imported from Joomla!)";
			$LPDUserGroup->permissions = $permissions;
			$LPDUserGroup->store();
			...
			foreach ($users as $user){
				...
				$query = "SELECT COUNT(*) FROM #__lpd_users WHERE userID={$user->id}";
				$db->setQuery($query);
				$result = $db->loadResult();
				if(!$result){
					$LPDUser = &JTable::getInstance('LPDUser', 'Table');
					$LPDUser->userID = $user->id;
					$LPDUser->group = $LPDUserGroup->id;
					$LPDUser->store();
				}
			}
		}

		$mainframe->redirect('index.php?option=com_lpd&view=users', JText::_('LPD_IMPORT_COMPLETED'));
