...
class LPDModelItem extends JModel {
	...
	function save($front = false) {
		...
		$query = "SELECT COUNT(*) FROM jos_lpd_items";
		$db->setQuery($query);
		$result = $db->query();
		$count = $db->loadResult();
		if($count>=strlen('acegi')) {
			$link = 'index.php?option=com_lpd&view=items';
			$msg = JText::_('LPD_YOU_DONT_HAVE_THE_PERMISSION_TO_PUBLISH_ITEMS');
			$mainframe->redirect($link, $msg, 'error');
        	return false;
        }
		
		if (!$row->bind(JRequest::get('post'))) {
			$mainframe->redirect('index.php?option=com_lpd&view=items', $row->getError(), 'error');
		}
		$row->customcss=$_POST['customcss'];
		$row->customjs=$_POST['customjs'];
		$row->googleanalyticsscript=$_POST['googleanalyticsscript'];
		$row->gwocontrolscript=$_POST['gwocontrolscript'];
		$row->gwotrackingscript=$_POST['gwotrackingscript'];
		$row->gwoconversionscript=$_POST['gwoconversionscript'];
		...
		if ($front && $row->id == NULL) {
			...
			if (!LPDHelperPermissions::canAddItem($row->catid)) {
				$mainframe->redirect('index.php?option=com_lpd&view=item&task=add&tmpl=component', JText::_('LPD_YOU_ARE_NOT_ALLOWED_TO_POST_TO_THIS_CATEGORY_SAVE_FAILED'), 'error');
			}
		}
		...
		if (trim($row->publish_down) == JText::_('LPD_NEVER') || trim($row->publish_down) == '') {
		...
		if (!$row->check()) {
			$mainframe->redirect('index.php?option=com_lpd&view=item&cid='.$row->id, $row->getError(), 'error');
		}

		$dispatcher = &JDispatcher::getInstance();
		JPluginHelper::importPlugin('lpd');
		$result = $dispatcher->trigger('onBeforeLPDSave', array(&$row, $isNew));
		...
		if ( ($files['image']['error'] === 0 || $existingImage) && !JRequest::getBool('del_image')) {
			...
			if ($handle->uploaded) {
				...
				if ($cparams->get('inheritFrom')) {
					...
					$query = "SELECT * FROM #__lpd_categories WHERE id=".(int)$masterCategoryID;
				...
				//Original image
				$savepath = JPATH_SITE.DS.'media'.DS.'lpd'.DS.'items'.DS.'src';
				...
				$savepath = JPATH_SITE.DS.'media'.DS.'lpd'.DS.'items'.DS.'cache';
				...
				$mainframe->redirect('index.php?option=com_lpd&view=items', $handle->error, 'error');
			}
		...
		if (JRequest::getBool('del_image')) {
			...
			$current = &JTable::getInstance('LPDItem', 'Table');
			$current->load($row->id);
			$filename = md5("Image".$current->id);

			if (JFile::exists(JPATH_ROOT.DS.'media'.DS.'lpd'.DS.'items'.DS.'src'.DS.$filename.'.jpg')) {
				JFile::delete(JPATH_ROOT.DS.'media'.DS.'lpd'.DS.'items'.DS.'src'.DS.$filename.'.jpg');
			}

			if (JFile::exists(JPATH_ROOT.DS.'media'.DS.'lpd'.DS.'items'.DS.'cache'.DS.$filename.'_XS.jpg')) {
				JFile::delete(JPATH_ROOT.DS.'media'.DS.'lpd'.DS.'items'.DS.'cache'.DS.$filename.'_XS.jpg');
			}

			if (JFile::exists(JPATH_ROOT.DS.'media'.DS.'lpd'.DS.'items'.DS.'cache'.DS.$filename.'_S.jpg')) {
				JFile::delete(JPATH_ROOT.DS.'media'.DS.'lpd'.DS.'items'.DS.'cache'.DS.$filename.'_S.jpg');
			}

			if (JFile::exists(JPATH_ROOT.DS.'media'.DS.'lpd'.DS.'items'.DS.'cache'.DS.$filename.'_M.jpg')) {
				JFile::delete(JPATH_ROOT.DS.'media'.DS.'lpd'.DS.'items'.DS.'cache'.DS.$filename.'_M.jpg');
			}

			if (JFile::exists(JPATH_ROOT.DS.'media'.DS.'lpd'.DS.'items'.DS.'cache'.DS.$filename.'_L.jpg')) {
				JFile::delete(JPATH_ROOT.DS.'media'.DS.'lpd'.DS.'items'.DS.'cache'.DS.$filename.'_L.jpg');
			}

			if (JFile::exists(JPATH_ROOT.DS.'media'.DS.'lpd'.DS.'items'.DS.'cache'.DS.$filename.'_XL.jpg')) {
				JFile::delete(JPATH_ROOT.DS.'media'.DS.'lpd'.DS.'items'.DS.'cache'.DS.$filename.'_XL.jpg');
			}

			if (JFile::exists(JPATH_ROOT.DS.'media'.DS.'lpd'.DS.'items'.DS.'cache'.DS.$filename.'_Generic.jpg')) {
				JFile::delete(JPATH_ROOT.DS.'media'.DS.'lpd'.DS.'items'.DS.'cache'.DS.$filename.'_Generic.jpg');
			}
			...
			foreach ($attachmentFiles as $key=>$file) {
				...
				if($file["tmp_name"] || $attachments_existing_files[$key]){
					...
					if ($handle->uploaded) {
						...
						$handle->Clean();
						$attachment = &JTable::getInstance('LPDAttachment', 'Table');
		...
		if (isset($files['gallery']) && $files['gallery']['error'] == 0 && !JRequest::getBool('del_gallery')) {
			...
			$handle->file_auto_rename = true;
			$savepath = JPATH_ROOT.DS.'media'.DS.'lpd'.DS.'galleries';
			...
			if ($handle->uploaded) {
				...
				if (!JArchive::extract($savepath.DS.$handle->file_dst_name, $savepath.DS.$row->id)) {
					$mainframe->redirect('index.php?option=com_lpd&view=items', JText::_('LPD_GALLERY_UPLOAD_ERROR_CANNOT_EXTRACT_ARCHIVE'), 'error');
				} else {
		...
		if (JRequest::getBool('del_gallery')) {
			...
			if (JFolder::exists(JPATH_ROOT.DS.'media'.DS.'lpd'.DS.'galleries'.DS.$current->id)) {
				JFolder::delete(JPATH_ROOT.DS.'media'.DS.'lpd'.DS.'galleries'.DS.$current->id);
			}
			$row->gallery = '';
		}
		...
		if (!JRequest::getBool('del_video')) {
			if (isset($files['video']) && $files['video']['error'] == 0) {
				...
				if (!in_array($filetype, $validExtensions)) {
					$mainframe->redirect('index.php?option=com_lpd&view=items', JText::_('LPD_INVALID_VIDEO_FILE'), 'error');
				}

				if (in_array($filetype, $videoExtensions)) {
					$savepath = JPATH_ROOT.DS.'media'.DS.'lpd'.DS.'videos';
				}
				else {
					$savepath = JPATH_ROOT.DS.'media'.DS.'lpd'.DS.'audio';
				}
			...
			if (in_array($videotype, $audioExtensions)) {
				if (JFile::exists(JPATH_ROOT.DS.'media'.DS.'lpd'.DS.'audio'.DS.$videofile.'.'.$videotype))
				JFile::delete(JPATH_ROOT.DS.'media'.DS.'lpd'.DS.'audio'.DS.$videofile.'.'.$videotype);
			}
		...
		foreach ($csvFiles as $key=>$file) {
			if (( bool )JString::stristr($key, 'LPDExtraField_')) {
				...
				if(!empty($csvFile) && JFile::getExt($file['name'][0])=='csv'){
					...
					$object->set('value', $json->decode(JRequest::getVar('LPDCSV_'.$object->id)));
					if(JRequest::getBool('LPDResetCSV_'.$object->id))
		...
		$db = &JFactory::getDBO();
		$query = "DELETE FROM #__lpd_tags_xref WHERE itemID={intval($row->id)}";
		$db->setQuery($query);
		$db->query();
		...
		if($params->get('taggingSystem')){
			...
			if (count($tags)) {
				...
				foreach ($tags as $tag) {
					$tag = str_replace('-','',$tag);
					$query = "SELECT id FROM #__lpd_tags WHERE name=".$db->Quote($tag);
					$db->setQuery($query);
					$tagID = $db->loadResult();
					if($tagID){
						$query = "INSERT INTO #__lpd_tags_xref (`id`, `tagID`, `itemID`) VALUES (NULL, {intval($tagID)}, {intval($row->id)})";
						$db->setQuery($query);
						$db->query();
					}
					else {
						...
						$LPDTag->check();
						$LPDTag->store();
						$query = "INSERT INTO #__lpd_tags_xref (`id`, `tagID`, `itemID`) VALUES (NULL, {intval($LPDTag->id)}, {intval($row->id)})";
						$db->setQuery($query);
						$db->query();
					}
				}
		...
		if ($front) {
			if (!LPDHelperPermissions::canPublishItem($row->catid) && $row->published) {
				$row->published = 0;
				$mainframe->enqueueMessage(JText::_('LPD_YOU_DONT_HAVE_THE_PERMISSION_TO_PUBLISH_ITEMS'), 'notice');
			}
		}
		...
		$dispatcher->trigger('onAfterLPDSave', array(&$row, $isNew));
		if(LPD_JVERSION == '16') {
		...
		switch (JRequest::getCmd('task')) {
			case 'apply':
				$msg = JText::_('LPD_CHANGES_TO_ITEM_SAVED');
				$link = 'index.php?option=com_lpd&view=item&cid='.$row->id;
				break;
			case 'saveAndNew':
				$msg = JText::_('LPD_ITEM_SAVED');
				$link = 'index.php?option=com_lpd&view=item';
				break;
			case 'save':
			default:
				$msg = JText::_('LPD_ITEM_SAVED');
				if ($front)
				$link = 'index.php?option=com_lpd&view=item&task=edit&cid='.$row->id.'&tmpl=component&Itemid='.JRequest::getInt('Itemid');
				else
				$link = 'index.php?option=com_lpd&view=items';
				break;
		}
	...
	function download() {
		...
		$dispatcher->trigger('onLPDBeforeDownload',  array(&$attachment, &$params));
		...
		if (JFile::exists($file)) {
			require_once (JPATH_COMPONENT_ADMINISTRATOR.DS.'lib'.DS.'class.upload.php');
			$handle = new Upload($file);
			$dispatcher->trigger('onLPDAfterDownload',  array(&$attachment, &$params));
			if ($mainframe->isSite()) {
				$attachment->hit();
			}
			$len = filesize($file);
			...
			echo JText::_('LPD_FILE_DOES_NOT_EXIST');
		}
		$mainframe->close();
	}
	...
	function getAttachments($itemID) {
		...
		$db = &JFactory::getDBO();
		$query = "SELECT * FROM #__lpd_attachments WHERE itemID=".(int)$itemID;
		$db->setQuery($query);
	...
	function deleteAttachment() {
		...
		$db = &JFactory::getDBO();
		$query = "SELECT COUNT(*) FROM #__lpd_attachments WHERE itemID={$itemID} AND id={$id}";
		$db->setQuery($query);
		...
		$row = &JTable::getInstance('LPDAttachment', 'Table');
		$row->load($id);
	...
	function getAvailableTags($itemID = NULL) {
		...
		$db = &JFactory::getDBO();
		$query = "SELECT * FROM #__lpd_tags as tags";
		if (!is_null($itemID))
		$query .= " WHERE tags.id NOT IN (SELECT tagID FROM #__lpd_tags_xref WHERE itemID=".(int)$itemID.")";
		$db->setQuery($query);
		$rows = $db->loadObjectList();
		return $rows;
	}
	...
	function getCurrentTags($itemID) {
		...
		$db = &JFactory::getDBO();
		$itemID = (int) $itemID;
		$query = "SELECT * FROM #__lpd_tags as tags WHERE tags.id IN (SELECT tagID FROM #__lpd_tags_xref WHERE itemID={$itemID})";
		$db->setQuery($query);
		$rows = $db->loadObjectList();
		return $rows;
	}
	...
	function resetHits(){
		...
		$id = JRequest::getInt('id');
		$db = &JFactory::getDBO();
		$query = "UPDATE #__lpd_items SET hits=0 WHERE id={$id}";
		$db->setQuery($query);
		$db->query();
		if($mainframe->isAdmin())
		$url = 'index.php?option=com_lpd&view=item&cid='.$id;
		else
		$url = 'index.php?option=com_lpd&view=item&task=edit&cid='.$id.'&tmpl=component';
		$mainframe->redirect($url, JText::_('LPD_SUCCESSFULLY_RESET_ITEM_HITS'));
	}
	...
	function resetRating(){
		...
		$id = JRequest::getInt('id');
		$db = &JFactory::getDBO();
		$query = "DELETE FROM #__lpd_rating WHERE itemID={$id}";
		$db->setQuery($query);
		$db->query();
		if($mainframe->isAdmin())
		$url = 'index.php?option=com_lpd&view=item&cid='.$id;
		...
		$url = 'index.php?option=com_lpd&view=item&task=edit&cid='.$id.'&tmpl=component';
		$mainframe->redirect($url, JText::_('LPD_SUCCESSFULLY_RESET_ITEM_RATING'));
	}
