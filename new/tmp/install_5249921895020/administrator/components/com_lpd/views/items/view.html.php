...
class LPDViewItems extends JView
{
	function display($tpl = null) {
		...
		$options = array();
		$options[] = JHTML::_('select.option', 0, '- '. JText::_('LPD_NO_USER') .' -');
		foreach($authors as $author){
		    $name = $author->name;
		    if($author->block){
		        $name.= ' ['.JText::_('LPD_USER_DISABLED').']';
		    }
		...
		$filter_state_options[] = JHTML::_('select.option', -1, JText::_('LPD_SELECT_PUBLISHING_STATE'));
		...
		$filter_featured_options[] = JHTML::_('select.option', -1, JText::_('LPD_SELECT_FEATURED_STATE'));
		$filter_featured_options[] = JHTML::_('select.option', 1, JText::_('LPD_FEATURED'));
		$filter_featured_options[] = JHTML::_('select.option', 0, JText::_('LPD_NOT_FEATURED'));
		...
		if($params->get('showTagFilter')){
		    ...
		    $tagsModel= new LPDModelTags;
		    $options = $tagsModel->getFilter();
		    $option = new JObject();
		    $option->id = 0;
		    $option->name = JText::_('LPD_SELECT_TAG');
		...
		JToolBarHelper::title( JText::_('LPD_ITEMS'), 'lpd.png' );
		if ($filter_trash == 1) {
			JToolBarHelper::custom('restore','restore.png','restore_f2.png','LPD_RESTORE', true);
			JToolBarHelper::deleteList('LPD_ARE_YOU_SURE_YOU_WANT_TO_DELETE_SELECTED_ITEMS', 'remove', 'LPD_DELETE');
		}
		else{
			...
			JToolBarHelper::customX( 'featured', 'default.png', 'default_f2.png', 'LPD_TOGGLE_FEATURED_STATE' );
...
/*		if ($user->gid > 23 && !$params->get('hideImportButton')){
			$buttonUrl = JURI::base().'index.php?option=com_lpd&amp;view=items&amp;task=import';
			$buttonText = JText::_('LPD_IMPORT_JOOMLA_CONTENT');
			$button	= '<a id="LPDImportContentButton" href="'.$buttonUrl.'"><span class="icon-32-archive" title="'.$buttonText.'"></span>'.$buttonText.'</a>';
	...
	function move(){
		...
		JToolBarHelper::title( JText::_('LPD_MOVE_ITEMS'), 'lpd.png' );
