...
class JElementMenuItem extends JElement
{
	...
	function fetchElement($name, $value, &$node, $control_name)
	{
		...
		$options 	= array();
		$options[]	= JHTML::_('select.option', '', '- '.JText::_('LPD_SELECT_MENU_ITEM').' -');
		...
		foreach ($menuTypes as $type)
		{
			if ($type != '')
			{
				...
				$options[]	= JHTML::_('select.option',  $type->menutype, $type->title . ' - ' . JText::_('LPD_TOP'), 'value', 'text', true );
			}
			if (isset( $groupedList[$type->menutype] ))
			{
				...
				for ($i = 0; $i < $n; $i++)
				{
					...
					if($item->published==0) $item->treename.=' [**'.JText::_('LPD_UNPUBLISHED').'**]';
					if($item->published==-2) $item->treename.=' [**'.JText::_('LPD_TRASHED').'**]';
