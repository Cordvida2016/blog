...
class JElementItem extends JElement {
	...
	function fetchElement($name, $value, & $node, $control_name) {
		...
		if ($value) {
			...
			$item->title = JText::_('LPD_SELECT_AN_ITEM');
		}
		...
		$link = 'index.php?option=com_lpd&amp;view=items&amp;task=element&amp;tmpl=component&amp;object='.$name;
		...
		<div class="button2-left">
			<div class="blank">
				<a class="modal" title="'.JText::_('LPD_SELECT_AN_ITEM').'"  href="'.$link.'" rel="{handler: \'iframe\', size: {x: 700, y: 450}}">'.JText::_('LPD_SELECT').'</a>
			</div>
		</div>
