<?php
/**
 * @version		$Id: lpd.php 1383 2011-12-05 12:10:24Z lefteris.kavadas $
...
class plgSearchLPD extends JPlugin {
	...
	function onSearchAreas() {
		JPlugin::loadLanguage('plg_search_lpd', JPATH_ADMINISTRATOR);
		static $areas = array('lpd'=>'LPD_ITEMS');
		return $areas;
	}
	...
	function onSearch($text, $phrase = '', $ordering = '', $areas = null) {
		...
		require_once (JPATH_SITE.DS.'components'.DS.'com_lpd'.DS.'helpers'.DS.'route.php');
		...
		$plugin = &JPluginHelper::getPlugin('search', 'lpd');
		...
		if ($limit> 0){
			...
			if($pluginParams->get('search_tags')){
				...
				$query = "SELECT id FROM #__lpd_tags WHERE MATCH(name) AGAINST ({$tagQuery} IN BOOLEAN MODE) AND published=1";
				$db->setQuery($query);
				$tagIDs = $db->loadResultArray();

				if(count($tagIDs)){
					JArrayHelper::toInteger($tagIDs);
					$query = "SELECT itemID FROM #__lpd_tags_xref WHERE tagID IN (".implode(',',$tagIDs).")";
					$db->setQuery($query);
    	...
    	FROM #__lpd_items AS i
    	INNER JOIN #__lpd_categories AS c ON c.id=i.catid AND c.access {$accessCheck}
		WHERE (";
	    ...
	    if(LPD_JVERSION == '16' && $mainframe->getLanguageFilter()) {
		...
		$query .= " GROUP BY i.id ";
			...
			if (isset($list)) {
				foreach ($list as $key=>$item) {
					$list[$key]->href = JRoute::_(LPDHelperRoute::getItemRoute($item->slug, $item->catslug));
