...
class LPDHelperUtilities {
	...
	function getAvatar($userID, $email = NULL, $width = 50) {
		...
		if(JFile::exists(JPATH_SITE.DS.'templates'.DS.$mainframe->getTemplate().DS.'images'.DS.'placeholder'.DS.'user.png')) {
			...
			$avatarPath = 'components/com_lpd/images/placeholder/user.png';
		}
			
		// Continue with default LPD avatar determination
		if ($userID == 'alias') {
			...
			JModel::addIncludePath(JPATH_SITE.DS.'components'.DS.'com_lpd'.DS.'models');
			...
			if ( empty($avatar)) {
				...
				$avatar = JURI::root(true).'/media/lpd/users/'.$avatar;
			}
	...
	function getCategoryImage($image, $params) {
		...
		$mainframe = &JFactory::getApplication();
		if (!empty($image)) {
			$categoryImage = JURI::root(true).'/media/lpd/categories/'.$image;
		}
		else {
			if ($params->get('catImageDefault')) {
				if(JFile::exists(JPATH_SITE.DS.'templates'.DS.$mainframe->getTemplate().DS.'images'.DS.'placeholder'.DS.'category.png')) {
					...
					$categoryImage = JURI::root(true).'/components/com_lpd/images/placeholder/category.png';
				}
	...
	function writtenBy($gender) {
		if (is_null($gender))
		return JText::_('LPD_WRITTEN_BY');
		if ($gender == 'm')
		return JText::_('LPD_WRITTEN_BY_MALE');
		if ($gender == 'f')
		return JText::_('LPD_WRITTEN_BY_FEMALE');
	}
