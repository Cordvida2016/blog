...
<!-- Start LPD Latest Layout -->
<div id="lpdContainer" class="latestView<?php if($this->params->get('pageclass_sfx')) echo ' '.$this->params->get('pageclass_sfx'); ?>">
	...
	<div class="latestItemsContainer" style="width:<?php echo number_format(100/$this->params->get('latestItemsCols'), 1); ?>%;">
		...
		<!-- Start LPD Category block -->
		<div class="latestItemsCategory">
			<?php if($this->params->get('categoryFeed')): ?>
			<!-- RSS feed icon -->
			<div class="lpdFeedIcon">
				<a href="<?php echo $category->feed; ?>" title="<?php echo JText::_('LPD_SUBSCRIBE_TO_THIS_RSS_FEED'); ?>">
			...
			<div class="latestItemsCategoryImage">
				<img src="<?php echo $category->image; ?>" alt="<?php echo LPDHelperUtilities::cleanHtml($category->name); ?>" style="width:<?php echo $this->params->get('catImageWidth'); ?>px;height:auto;" />
			</div>
			<?php endif; ?>
			...
			<!-- LPD Plugins: LPDCategoryDisplay -->
			<?php echo $category->event->LPDCategoryDisplay; ?>
			<div class="clr"></div>
		</div>
		<!-- End LPD Category block -->
		<?php endif; ?>
		...
		<!-- Start LPD User block -->
		<div class="latestItemsUser">
	
			<?php if($this->params->get('userFeed')): ?>
			<!-- RSS feed icon -->
			<div class="lpdFeedIcon">
				<a href="<?php echo $user->feed; ?>" title="<?php echo JText::_('LPD_SUBSCRIBE_TO_THIS_RSS_FEED'); ?>">
					<span><?php echo JText::_('LPD_SUBSCRIBE_TO_THIS_RSS_FEED'); ?></span>
				</a>
				<div class="clr"></div>
			</div>
			...
			<p class="latestItemsUserAdditionalInfo">
				...
				<span class="latestItemsUserURL">
					<?php echo JText::_('LPD_WEBSITE_URL'); ?>: <a rel="me" href="<?php echo $user->profile->url; ?>" target="_blank"><?php echo $user->profile->url; ?></a>
				</span>
				<?php endif; ?>
	
				<?php if ($this->params->get('userEmail')): ?>
				<span class="latestItemsUserEmail">
					<?php echo JText::_('LPD_EMAIL'); ?>: <?php echo JHTML::_('Email.cloak', $user->email); ?>
				</span>
				<?php endif; ?>
			</p>
			<?php endif; ?>
			...
			<?php echo $user->event->LPDUserDisplay; ?>
		...
		<?php if($this->params->get('latestItemsDisplayEffect')=="first"): ?>
			...
			<?php foreach ($block->items as $itemCounter=>$item): LPDHelperUtilities::setDefaultImage($item, 'latest', $this->params); ?>
		...
		<?php else: ?>
			...
			<?php foreach ($block->items as $item): LPDHelperUtilities::setDefaultImage($item, 'latest', $this->params); ?>
			<?php $this->item=$item; echo $this->loadTemplate('item'); ?>
			<?php endforeach; ?>
