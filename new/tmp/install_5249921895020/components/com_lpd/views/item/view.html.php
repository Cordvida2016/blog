...
class LPDViewItem extends JView {
	...
	function display($tpl = null) {
		...
		// User LPD plugins
		$item->event->LPDUserDisplay = '';
		if (isset($item->author) && is_object($item->author->profile) && isset($item->author->profile->id)) {
			$dispatcher = &JDispatcher::getInstance();
			JPluginHelper::importPlugin('lpd');
			$results = $dispatcher->trigger('onLPDUserDisplay', array(&$item->author->profile, &$params, $limitstart));
			$item->event->LPDUserDisplay = trim(implode("\n", $results));
		}
		...
		if ($item->params->get('itemComments')) {
			...
			if(empty($item->event->LPDCommentsCounter) && empty($item->event->LPDCommentsBlock)){
				...
				if (!JRequest::getInt('print') && ($item->params->get('comments') == '1' || ($item->params->get('comments') == '2' && LPDHelperPermissions::canAddComment($item->catid)))) {
		...
		if ($item->metadesc) {
			...
			$metaDescItem = LPDHelperUtilities::characterLimit($metaDescItem, $params->get('metaDescLimit', 150));
		...
		// Add Custom CSS by janunez
		if ($item->customcss) {
			$document->addStyleDeclaration($item->customcss);
		}
		// Add Custom JavaScript by janunez
		if ($item->customjs) {
			$item->customjs=str_replace('<script type="text/javascript">', '', $item->customjs);
			$item->customjs=str_replace('</script>', '', $item->customjs);			
			$document->addScriptDeclaration($item->customjs);
		}
		// Add Google Analytics Script by janunez
		if ($item->googleanalyticsscript) {
			$item->googleanalyticsscript=str_replace('<script type="text/javascript">', '', $item->googleanalyticsscript);
			$item->googleanalyticsscript=str_replace('</script>', '', $item->googleanalyticsscript);
			$document->addScriptDeclaration($item->googleanalyticsscript);
		}
		// Set Generator by janunez
		$document->setMetadata('generator', 'LandingPagesDesigner.com');
