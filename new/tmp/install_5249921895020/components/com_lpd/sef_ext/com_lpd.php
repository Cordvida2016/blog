<?php
/**
 * @version		$Id: com_lpd.php 1386 2011-12-07 11:01:31Z lefteris.kavadas $
...
if (!function_exists('getCategoryPath')) {
  ...
  function getCategoryPath($catid, $begin = false) {
    ...
    $catid = (int) $catid;
    $db = &JFactory::getDBO();
    $query = "SELECT * FROM #__lpd_categories WHERE id={$catid} AND published=1";
...
$sh404SefModLPDContentFeedAlias = $params->get('sh404SefModLPDContentFeedAlias', 'feed');
    ...
    if (isset($id) && $id > 0 && $task != 'download') {
		$id = (int)$id;
			if (!shTranslateUrl($option, $shLangName)) {
				$query = 'SELECT '.$sh404SefTitleAlias.', catid FROM #__lpd_items WHERE id = '.$id;
			} else {
				$query = 'SELECT id, '.$sh404SefTitleAlias.', catid FROM #__lpd_items WHERE id = '.$id;
			}
  ...
  case 'itemlist':
    ...
    switch ($task) {
      ...
      case 'module':
        ...
        if($sh404SefModLPDContentFeedAlias) {
        	$title[] = $sh404SefModLPDContentFeedAlias;
        }
        $title[] = $moduleTitle;
        break;
