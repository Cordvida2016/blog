...
class LPDModelItemlist extends JModel {
		...
		function getData($ordering = NULL) {
				...
				$query.=" FROM #__lpd_items as i LEFT JOIN #__lpd_categories AS c ON c.id = i.catid";

				if ($ordering == 'best')
						$query .= " LEFT JOIN #__lpd_rating r ON r.itemID = i.id";

				//Changed the query for the tag case for better performance
				//if ($task == 'tag')
				//		$query .= " LEFT JOIN #__lpd_tags_xref AS tags_xref ON tags_xref.itemID = i.id LEFT JOIN #__lpd_tags AS tags ON tags.id = tags_xref.tagID";
				...
				switch ($task) {
						...
						case 'category':
								...
								if ($cparams->get('inheritFrom')) {
										...
										$parent = &JTable::getInstance('LPDCategory', 'Table');
								...
								if ($cparams->get('catCatalogMode')) {
										...
										$categories = LPDModelItemlist::getCategoryTree($id);
						...
						case 'search':
								...
								$sql = LPDModelItemlist::prepareSearch($search);
								if (! empty($sql)) {
										$query .= $sql;
								} else {
										$rows = array();
						...
						case 'tag':
								...
								if (JFile::exists(JPATH_ADMINISTRATOR.DS.'components'.DS.'com_joomfish'.DS.'joomfish.php') && $task == 'tag') {
										...
										$sql .= " AND jfc.reference_table = 'lpd_tags'";
								...
								if (!isset($result) || $result < 1) {
										$sql = "SELECT id FROM #__lpd_tags WHERE name=".$db->Quote($tag);
										$db->setQuery($sql, 0, 1);
										$result = $db->loadResult();
								}
								
								$query.=" AND i.id IN (SELECT itemID FROM #__lpd_tags_xref WHERE tagID=".(int)$result.")";
						...
						default:
								...
								if (is_array($searchIDs) && count($searchIDs)) {
										...
										if ($params->get('catCatalogMode')) {
												...
												$result = LPDModelItemlist::getCategoryTree($searchIDs);
												if (count($result)) {
														$sql = @implode(',', $result);
														$query .= " AND i.catid IN ({$sql})";
												}
		...
		function getTotal() {
				...
				$query = "SELECT COUNT(*) FROM #__lpd_items as i LEFT JOIN #__lpd_categories c ON c.id = i.catid";

				if ($task == 'tag')
						$query .= " LEFT JOIN #__lpd_tags_xref tags_xref ON tags_xref.itemID = i.id LEFT JOIN #__lpd_tags tags ON tags.id = tags_xref.tagID";
		...
		function getCategoryTree($categories){
			...
			static $LPDCategoryTreeInstances = array();
			if(isset($LPDCategoryTreeInstances[$clientID]) && array_key_exists($key, $LPDCategoryTreeInstances[$clientID])){
				return $LPDCategoryTreeInstances[$clientID][$key];
			}
			$array = $categories;
			while(count($array)){
				$query = "SELECT id
						FROM #__lpd_categories 
			...
			JArrayHelper::toInteger($categories);
			$categories = array_unique($categories);
			$LPDCategoryTreeInstances[$clientID][$key] = $categories;
			return $categories;
		}
		...
		function getCategoryChildren($catid, $clear = false) {
			...
			$catid = (int) $catid;
			$db = &JFactory::getDBO();
			$query = "SELECT * FROM #__lpd_categories WHERE parent={$catid} AND published=1 AND trash=0 AND access<={$aid} ORDER BY ordering ";
			$db->setQuery($query);
			$rows = $db->loadObjectList();

			foreach ($rows as $row) {
				array_push($array, $row->id);
				if (LPDModelItemlist::hasChildren($row->id)) {
					LPDModelItemlist::getCategoryChildren($row->id);
				}
			}
			return $array;
		}
		...
		function hasChildren($id) {
			...
			$id = (int) $id;
			$db = &JFactory::getDBO();
			$query = "SELECT * FROM #__lpd_categories WHERE parent={$id} AND published=1 AND trash=0 AND access<={$aid} ";
			$db->setQuery($query);
			$rows = $db->loadObjectList();
		...
		function getCategoryFirstChildren($catid, $ordering = NULL) {
				...
				$aid = $user->get('aid');
				$db = &JFactory::getDBO();
				$query = "SELECT * FROM #__lpd_categories WHERE parent={$catid} AND published=1 AND trash=0";
		...
		function countCategoryItems($id) {
				...
				$categories = LPDModelItemlist::getCategoryTree($id);
				$query = "SELECT COUNT(*) FROM #__lpd_items WHERE catid IN (".implode(',', $categories).") AND published=1 AND trash=0";
		...
		function getAuthorLatest($itemID, $limit, $userID) {
				...
				$query = "SELECT i.*, c.alias as categoryalias FROM #__lpd_items as i 
				LEFT JOIN #__lpd_categories c ON c.id = i.catid 
				WHERE i.id != {$itemID} 
				AND i.published = 1 
				...
				foreach($rows as $item) {
					...
					if (JFile::exists(JPATH_SITE.DS.'media'.DS.'lpd'.DS.'items'.DS.'cache'.DS.md5("Image".$item->id).'_XS.jpg'))
							$item->imageXSmall = JURI::root(true).'/media/lpd/items/cache/'.md5("Image".$item->id).'_XS.jpg';

					if (JFile::exists(JPATH_SITE.DS.'media'.DS.'lpd'.DS.'items'.DS.'cache'.DS.md5("Image".$item->id).'_S.jpg'))
							$item->imageSmall = JURI::root(true).'/media/lpd/items/cache/'.md5("Image".$item->id).'_S.jpg';

					if (JFile::exists(JPATH_SITE.DS.'media'.DS.'lpd'.DS.'items'.DS.'cache'.DS.md5("Image".$item->id).'_M.jpg'))
							$item->imageMedium = JURI::root(true).'/media/lpd/items/cache/'.md5("Image".$item->id).'_M.jpg';

					if (JFile::exists(JPATH_SITE.DS.'media'.DS.'lpd'.DS.'items'.DS.'cache'.DS.md5("Image".$item->id).'_L.jpg'))
							$item->imageLarge = JURI::root(true).'/media/lpd/items/cache/'.md5("Image".$item->id).'_L.jpg';

					if (JFile::exists(JPATH_SITE.DS.'media'.DS.'lpd'.DS.'items'.DS.'cache'.DS.md5("Image".$item->id).'_XL.jpg'))
							$item->imageXLarge = JURI::root(true).'/media/lpd/items/cache/'.md5("Image".$item->id).'_XL.jpg';

					if (JFile::exists(JPATH_SITE.DS.'media'.DS.'lpd'.DS.'items'.DS.'cache'.DS.md5("Image".$item->id).'_Generic.jpg'))
							$item->imageGeneric = JURI::root(true).'/media/lpd/items/cache/'.md5("Image".$item->id).'_Generic.jpg';
				}
		...
		function getRelatedItems($itemID, $tags, $params) {
				...
				$query = "SELECT DISTINCT itemID FROM #__lpd_tags_xref WHERE tagID IN ({$sql}) AND itemID!={$itemID}";
 		$db->setQuery($query);
		...
		$query .= " AND c.trash = 0 
				...
				for ($key = 0; $key < sizeof($rows); $key++) {
					...
					LPDHelperUtilities::setDefaultImage($rows[$key], 'relatedByTag', $params);
				}
				return $rows;
		}
			...
			if (JFile::exists(JPATH_ADMINISTRATOR.DS.'components'.DS.'com_joomfish'.DS.'joomfish.php') && $currentLang != $defaultLang) {
				...
				if ($type=='exact') {
					...
					if (JString::strlen($word) > 3 && !in_array($word, $search_ignore)) {
						...
						$jfQuery .= " WHERE jfc.reference_table = 'lpd_items'";
		...
		function getModuleItems($moduleID) {
				...
				if (is_null($module))
						...
						switch ($module->module) {
								...
								case 'mod_lpd_content':
										require_once (JPATH_SITE.DS.'modules'.DS.'mod_lpd_content'.DS.'helper.php');
										$helper = new modLPDContentHelper;
										$items = $helper->getItems($params, $format);
										break;

								case 'mod_lpd_comments':
										if($params->get('module_usage')==1)
											JError::raiseError(404, JText::_('LPD_NOT_FOUND'));
											
										require_once (JPATH_SITE.DS.'modules'.DS.'mod_lpd_comments'.DS.'helper.php');
										$helper = new modLPDCommentsHelper;
										$items = $helper->getLatestComments($params);
										
										foreach($items as $item){
											$item->title = $item->userName.' '.JText::_('LPD_COMMENTED_ON').' '.$item->title;
